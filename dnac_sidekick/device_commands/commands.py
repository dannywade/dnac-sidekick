...
@click.command
@click.option("--device", default="", help="Specify a device's hostname to run commands.")
@click.option("--command", default="", help="Specify a command to run on the specified device.")
@click.pass_context
def command_runner(ctx, device, command):
    ...
    dnac_devices_url = f"{ctx.obj.dnac_url}/dna/intent/api/v1/network-device?hostname={device}"
    ...
    dnac_command_run = f"{ctx.obj.dnac_url}/dna/intent/api/v1/network-device-poller/cli/read-request"
    payload = {
        "timeout": 5,
    ...
    comm_run_resp = requests.post(url=dnac_command_run, headers=headers, json=payload, verify=False)
